...
//心跳
#define DEF_PACK_HEART_RQ       (DEF_PACK_BASE + 12)
...
//可以再添加一个结果 在线,不让相同名字用户登录,
//实现方法是 redis 写时限的key-value 然后 心跳包取判断在线更新时限,一直没更新到0 键自动清除.
...
//心跳请求
typedef struct STRU_HEART_RQ
{
    STRU_HEART_RQ()
    {
        m_nType = DEF_PACK_HEART_RQ;
        m_UserID = 0;
    }
    PackType   m_nType;   //包类型
    int m_UserID;
